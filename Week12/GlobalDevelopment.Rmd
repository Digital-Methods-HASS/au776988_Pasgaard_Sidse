...
ggplot(subset(gapminder, year == 2007), aes(gdpPercap, lifeExp, size = pop/1000000)) +
  geom_point(aes(color = continent)) +
  scale_x_log10() +
  ggtitle("World in 2007 (Figure 02)")+
...
The GDP varies a lot between countries, without log scale the poor countries get squished, and with log scale the data is spread more evenly and it is easier to compare. 

2. *Answer: In Figure 1: Who is the outlier (the richest country in 1952) far right on the x axis?* 
The answer is Kuwait in Asia with a GDP percap 108382.4
...
The five richest countries in 2007 are: Norway, Kuwait, Singapore, United States nad Ireland
...
anim + transition_states(year, 
                      transition_length = 1,
                      state_length = 1)+
  labs(title = "World from 1952–2007",
...
anim2 <- ggplot(gapminder, aes(gdpPercap, lifeExp, size = pop/1000000)) +
  ...
  labs(title = "World development over time",
    subtitle = "Year: {frame_time}",
    x = "GDP per capita",
    y = "Life expectancy")+
  ease_aes('linear')
animate(anim2, nframes = 300, fps = 25)
anim2
```
